...
Route::group(['prefix' => 'admin', 'middleware' => config('fortify.middleware', ['web'])], function () {
    ...
    if ($enableViews) {
        Route::get(RoutePath::for('admin-login', '/login'), [AdminAuthenticatedSessionController::class, 'createadmin'])
    ...
    Route::post(RoutePath::for('admin-login', '/login'), [AdminAuthenticatedSessionController::class, 'storeadmin'])
        ->middleware(array_filter([
            'guest:'.config('fortify.guard'),
            $limiter ? 'throttle:'.$limiter : null,
        ]))
        ->name('admin-formlogin');

    Route::post(RoutePath::for('admin-logout', '/logout'), [AuthenticatedSessionController::class, 'destroy'])
        ->name('admin-logout');
